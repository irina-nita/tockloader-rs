...
pub async fn install_app(
    ...
    if let Some(port) = probe_session.port.as_mut() {
        ...
        // Create app object
        let app = tab_file
            .extract_app(Some(arch.clone()))
            .unwrap();
        let header_binary = tab_file.extract_header_binary(Some(arch.clone()));
        dbg!(header_binary.len());
        let footer_binary = tab_file.extract_footer_binary(Some(arch.clone()));
